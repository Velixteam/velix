  ...
  return {
    ...
    setup(ctx: Record<string, unknown>) {
      ...
      if (typeof globalThis !== 'undefined') {
        (globalThis as unknown as { __VELIX_AI__: AIClient }).__VELIX_AI__ = client;
      }
    }
  };
}
...
export function useAI(): AIClient {
  if (typeof globalThis === 'undefined' || !(globalThis as unknown as { __VELIX_AI__?: AIClient }).__VELIX_AI__) {
    throw new Error('AI client not initialized. Make sure aiPlugin is configured.');
  }
  
  return (globalThis as unknown as { __VELIX_AI__: AIClient }).__VELIX_AI__;
}
 ...
 * ```
 */
export function createAIAction<TInput = unknown, TOutput = string>(config: {
  input: TInput;
