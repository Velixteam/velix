...
export type ServerAction<State, Payload = FormData> = (prevState: Awaited<State>, payload: Payload) => State | Promise<State>;
...
export type InferParams<Path extends string> = Path extends `${infer _Start}/[...${infer Param}]/${infer Rest}` ? {
    [K in Param]: string;
