...
export function Link({ href, prefetch = false, replace: shouldReplace = false, scroll = true, shallow = false, children, onClick, onMouseEnter, ...rest }: LinkProps) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window !== 'undefined' && (window as unknown as { __VELIX_DEV_TOOLS__?: { setStatus: (s: string) => void } }).__VELIX_DEV_TOOLS__) {
      (window as unknown as { __VELIX_DEV_TOOLS__: { setStatus: (s: string) => void } }).__VELIX_DEV_TOOLS__.setStatus('navigating');
    }
