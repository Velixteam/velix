 ...
 */
export function createIsland(Component: React.ComponentType<Record<string, unknown>>, options: { name?: string; clientPath?: string } = {}) {
  const { name = (Component as { name?: string }).name || 'Island', clientPath } = options;

  function IslandWrapper(props: Record<string, unknown>) {
    return Island({
 ...
 */
export function createLazyIsland(Component: React.ComponentType<Record<string, unknown>>, options: {
  name?: string; clientPath?: string; strategy?: string; media?: string | null;
} = {}) {
  const {
    name = (Component as { name?: string }).name || 'LazyIsland',
    clientPath,
    strategy = LoadStrategy.VISIBLE,
    media = null
  } = options;

  function LazyIslandWrapper(props: Record<string, unknown>) {
    const islandId = generateIslandId(name);
 ...
 */
export function generateHydrationScript(islands: Array<{ id: string; name: string; clientPath: string; props: Record<string, unknown> }>): string {
 ...
 */
export function generateAdvancedHydrationScript(islands: Array<{ id: string; name: string; clientPath: string; props: Record<string, unknown>; strategy?: string; media?: string | null }>): string {
  if (!islands.length) return '';
